...
func NewAuthMiddleware(opts *AuthOptions) func(h http.Handler) http.Handler {
	...
	return func(h http.Handler) http.Handler {
		fn := func(w http.ResponseWriter, r *http.Request) {
			...
			ccCleint := cc.GetCcClient(config.GetMFconfig().ServerConfig.CCAddress)
			...
			if err != nil || token == "" {
				...
				logger.Logger().Error("Authorization check failed, err：", err)
				logger.Logger().Error("CCAddress：", config.GetMFconfig().ServerConfig.CCAddress)
				logger.Logger().Error("Token：", token)
				return
			}
			ccToken = token
			ccIsSA = isSA
			...
			log.Debugf("%s: %v, %v", UserIDHeader, userID, ccIsSA)
