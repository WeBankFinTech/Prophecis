...
var (
	...
	// FIXME MLSS Change: keys for MLSS configs
...
const (
	RMB_ModelType_Logistic_Regression = "Logistic_Regression"
	RMB_ModelType_Decision_Tree       = "Decision_Tree"
	RMB_ModelType_Random_Forest       = "Random_Forest"
	RMB_ModelType_XGBoost             = "XGBoost"
	RMB_ModelType_LightGBM            = "LightGBM"
	RMB_ModelUsage_Classification     = "Classification"
	RMB_ModelUsage_Regression         = "Regression"
	RMB_PushEvent_Status_Creating     = "Creating"
	RMB_PushEvent_Status_Failed       = "Failed"
	RMB_PushEvent_Status_Success      = "Success" //Failed\Pushing\Success

	Push_Event_File_Type_Report = "DATA"
	Push_Event_File_Type_Model  = "MODEL"
)

const TimeFormat = "2006-01-02 15:04:05"
const Max_RW = 1024 * 1024 * 1

func GetMFconfig() MFConfig {
	return mfConfig
}

//func GetRmbConfig() *RmbConfig {
//	return &mfConfig.RmbConfig
//}
//
//func GetFpsConfig() *FpsConfig {
//	return &mfConfig.FpsConfig
//}

type MFConfig struct {
	DBConfig       DBConfig       `yaml:"DBConfig"`
	ServerConfig   ServerConfig   `yaml:"ServerConfig"`
	StorerConfig   StorerConfig   `yaml:"StorerConfig"`
	ResourceConfig ResourceConfig `yaml:"ResourceConfig"`
	//RmbConfig      RmbConfig      `yaml:"RmbConfig"`
	//FpsConfig      FpsConfig      `yaml:"FpsConfig"`
}

type ResourceConfig struct {
	ContainerEngineCpu    float64 `yaml:"containerEngineCpu"`
	ContainerEngineGpu    string  `yaml:"containerEngineGpu"`
	ContainerEngineMemory float64 `yaml:"containerEngineMemory"`
	IstioSideCarCpu       float64 `yaml:"istioSideCarCpu"`
	IstioSideCarGpu       string  `yaml:"istioSideCarGpu"`
	IstioSideCarMemory    float64 `yaml:"istioSideCarMemory"`
}
...
type ServerConfig struct {
	...
	CCAddress  string `yaml:"CCAddress"`
...
type StorerConfig struct {
	...
	StorerTls           string `yaml:"storerTls"`
	StorerServerEnvName string `yaml:"storerServerEnvName"`
	StorerServerName    string `yaml:"storerServerName"`
	StorerCA            string `yaml:"storerCA"`
}
...
//	SaveCnt  int    `yaml:"saveCnt"`
//}

//type RmbConfig struct {
...
func loadYaml() MFConfig {
	...
	yamlFile, err := ioutil.ReadFile("/etc/mlss/config.yml")
	//yamlFile, err := ioutil.ReadFile("D://config.yml")

	logger.Info("test:%v", string(yamlFile))
	logger.Info("Loading MF App Config...")
	if err != nil {
		logger.Error("YamlFile Get Error: %v", err)
	}
	err = yaml.Unmarshal(yamlFile, &conf)
	if err != nil {
		logger.Error("YamlFile Unmarshal Error: %v", err)
	}
...
// it is false.
func IsStorerTLSEnabled() bool {
	if strings.ToUpper(GetMFconfig().StorerConfig.StorerTls) == "TRUE" {
		return true
	}
	return false
}

func GetStorerCAKey() string {
	return "-----BEGIN CERTIFICATE-----\n" + GetMFconfig().StorerConfig.StorerCA + "\n-----END CERTIFICATE-----"
}

func GetEnvServerName() string {
	return GetMFconfig().StorerConfig.StorerServerEnvName
}

func GetStorerServerName() string {
	return GetMFconfig().StorerConfig.StorerServerName
}
