...
func normalizedFrameworkName(fw *grpc_storage.Framework) string {
	return strings.ToLower(fw.Name)
}

func getExternalVersions() (grpc_storage.Frameworks, error) {
	frameworks, err := framework.GetFrameworks(learnerConfigPath)
	if err != nil {
		return grpc_storage.Frameworks{}, err
	}
...
func convertToUserFramework(internalFramework framework.Frameworks) (grpc_storage.Frameworks, error) {
	var frameworks grpc_storage.Frameworks
...
func removeInternalFrameworks(frameworks grpc_storage.Frameworks) {
	for frameworkName, frameworkVersion := range frameworks.Frameworks {
		var externalFrameworks []*grpc_storage.FrameworkDetails
