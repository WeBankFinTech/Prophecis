...
// LockEntry represents which grpc_storage service currently has a lock on the queue
...
// grpc_storage should acquire the lock before calling Enqueue()
...
// grpc_storage should acquire the lock before calling Dequeue()
...
// grpc_storage should acquire the lock before calling Delete()
...
// grpc_storage should use this when pulling jobs, so that multiple trainers do not peek/submit the same job to lcm
