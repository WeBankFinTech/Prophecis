...
type TrainingRecord struct {
	...
	ModelDefinition       *grpc_storage.ModelDefinition `bson:"model_definition,omitempty" json:"model_definition"`
	Training              *grpc_storage.Training        `bson:"training,omitempty" json:"training"`
	Datastores            []*grpc_storage.Datastore     `bson:"data_stores,omitempty" json:"data_stores"`
	TrainingStatus        *grpc_storage.TrainingStatus  `bson:"training_status,omitempty" json:"training_status"`
	Metrics               *grpc_storage.Metrics         `bson:"metrics,omitempty" json:"metrics"`
	...
	//ParameterServer string `bson:"parameter_server,omitempty" json:"parameter_server,omitempty"`
	...
	StoreStatus string `bson:StoreStatus,Linkis logStoreStatus`
...
type JobHistoryEntry struct {
	...
	Status        grpc_storage.Status `bson:"status,omitempty" json:"status,omitempty"`
...
type FlowJsonEntry struct {
	ID               bson.ObjectId `bson:"_id,omitempty" json:"id"`
	ExecId           int64         `bson:"exec_id" json:"dss_flow_id"`
	DssFlowId        int64         `bson:"dss_flow_id" json:"dss_flow_id"`
	DssFlowVersionID string        `bson:"dss_flow_version_version,omitempty" json:"dss_flow_version_version,omitempty"`
	ExpJson          string        `bson:"exp_json,omitempty" json:"exp_json,omitempty"`
}

type Repository interface {
	Store(c *TrainingRecord) error
	Find(trainingID string) (*TrainingRecord, error)
	FindTrainingStatus(trainingID string) (*grpc_storage.TrainingStatus, error)
	FindTrainingStatusID(trainingID string) (grpc_storage.Status, error)
	...
	FindAll(userID string, page string, size string, expRunId string) (*TrainingRecordPaged, error)
	FindAllByUserIdAndNamespace(user *string, namespace *string, page string, size string, expRunId string) (*TrainingRecordPaged, error)
	FindAllByUserIdAndNamespaceList(user *string, namespace *[]string, page string, size string, clusterName string, expRunId string) (*TrainingRecordPaged, error)
	FindCurrentlyRunningTrainings(limit int) ([]*TrainingRecord, error)
	FindAllNotCompletedLinkisJobs() ([]*TrainingRecord, error)
...
type flowJsonRepository interface {
	AddFlowJson(FlowJsonEntry) error
	GetFlowJson(execId int64) (*FlowJsonEntry, error)
	Close()
}
...
func (r *trainingsRepository) FindTrainingStatus(trainingID string) (*grpc_storage.TrainingStatus, error) {
	...
	tr := &grpc_storage.TrainingStatus{}
...
func (r *trainingsRepository) FindTrainingStatusID(trainingID string) (grpc_storage.Status, error) {
	logr := logger.LocLogger(log.StandardLogger().WithField("module", "trainingRepository").WithField(logger.LogkeyTrainingID, trainingID))
	sess := r.session.Clone()
	defer sess.Close()
	...
	return grpc_storage.Status_NOT_STARTED, nil
}
...
func (r *trainingsRepository) FindAll(userID string, page string, size string, expRunId string) (*TrainingRecordPaged, error) {
	...
	queryMap := &bson.M{}
	var n []bson.M
	if reflect2.IsNil(expRunId) && len(expRunId) <= 0 {
		n = append(n, bson.M{"user_id": userID})
	} else {
		queryMap = &bson.M{"exp_run_id": expRunId}
		n = append(n, bson.M{"user_id": userID})
	}
	if len(n) != 0 {
		(*queryMap)["$or"] = n
...
// FIXME MLSS Change: get models filter by username and namespace
func (r *trainingsRepository) FindAllByUserIdAndNamespace(user *string, namespace *string, page string, size string, expRunId string) (*TrainingRecordPaged, error) {
	logr := logger.LocLogger(log.StandardLogger().WithField("module", "trainingRepository"))
	var tr []*TrainingRecord
	sess := r.session.Clone()
	defer sess.Close()
	...
	if *namespace != "" {
		n = append(n, bson.M{"namespace": namespace})
	}
	//expRunId
	//expRunId
	if !reflect2.IsNil(expRunId) && len(expRunId) > 0 {
...
func (r *trainingsRepository) FindAllByUserIdAndNamespaceList(user *string, namespaces *[]string, page string, size string, clusterName string, expRunId string) (*TrainingRecordPaged, error) {
	logr := logger.LocLogger(log.StandardLogger().WithField("module", "trainingRepository"))
	var tr []*TrainingRecord
	sess := r.session.Clone()
	defer sess.Close()
...
func (r *trainingsRepository) Delete(trainingID string) error {
	...
	var resources *grpc_storage.ResourceRequirements
	var status grpc_storage.TrainingStatus
	var framework *grpc_storage.Framework
	...
	newRecord := &TrainingRecord{
		...
		ModelDefinition: &grpc_storage.ModelDefinition{
			Framework: framework,
		},
		Training: &grpc_storage.Training{
			Resources: resources,
		},
...
func (r *trainingsRepository) FindAllNotCompletedLinkisJobs() ([]*TrainingRecord, error) {
	logr := logger.LocLogger(log.StandardLogger().WithField("module", "trainingRepository"))
	sess := r.session.Clone()
	defer sess.Close()
	...
	ms := bson.M{"job_type": "tfos", "$and": []bson.M{
...
func (r *trainingsRepository) AddFlowJson(f FlowJsonEntry) error {
	sess := r.session.Clone()
	defer sess.Close()

	var err error
	if f.ID == "" {
		err = sess.DB(r.database).C(r.collection).Insert(f)
	} else {
		err = sess.DB(r.database).C(r.collection).Update(bson.M{"_id": f.ID}, f)
	}
	if err != nil {
		logWith(strconv.FormatInt(f.DssFlowId, 10), f.DssFlowVersionID).Errorf("Error storing training record: %s", err.Error())
		//logWith(strconv.FormatInt(f.DssFlowId, 10), f.DssFlowVersionID).Errorf("Error storing training record: %s", err.Error())
		return err
	}
...
func (r *trainingsRepository) GetFlowJson(ExecId int64) (*FlowJsonEntry, error) {
	sess := r.session.Clone()
	defer sess.Close()

	tr := &FlowJsonEntry{}
	defer sess.Close()
	err := r.queryDatabase(&bson.M{"exec_id": ExecId}, sess).One(tr)
