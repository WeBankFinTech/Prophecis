...
func GetNewTrainingJobQueue(mongoURI string, database string, username string, password string,
	authenticationDatabase string, cert string, queueCollection string, lockCollection string) (*TrainingJobQueue, error) {
	return newTrainingJobQueue(mongoURI, database, username, password, authenticationDatabase,
		cert, queueCollection, lockCollection)
}
